"""
MegaBrainsFC Forward 1 (Left) Agent - AgentCore Runtime
Player ID: 3 | Position: FWD1 (Left Forward)
Zone: X[+5, +45], Y[-12, 0]
Home: X=+25, Y=-8
Role: Score goals from left side, create chances
"""
import json
import math
import os
...
ZONE_X_MAX = 45
ZONE_Y_MIN = -12
ZONE_Y_MAX = 0  # FWD1 fica no lado ESQUERDO (y negativo)
HOME_X = 25
HOME_Y = -8
...
OPP_GOAL_X = 50
OPP_GOAL_Y = 0
GOAL_WIDTH = 6  # goal from y=-3 to y=+3
...
# ==================== SYSTEM PROMPT ====================
SYSTEM_PROMPT = f"""You are an AI FORWARD LEFT (player {MY_PLAYER_ID}) in a 5v5 soccer match.
Your position is FWD1 (Left Forward Striker). Your JOB is to SCORE GOALS from the left side.
...
- Zone: X between {ZONE_X_MIN} and {ZONE_X_MAX}, Y between {ZONE_Y_MIN} and {ZONE_Y_MAX} (LEFT SIDE)
- Home base: X={HOME_X}, Y={HOME_Y}
- YOU ARE A STRIKER - your job is to SCORE and CREATE

======== TEAM ROSTER ========
- Player 0: GK (Goalkeeper) - at x=-48
- Player 1: DEF (Defender) - at x=-25
- Player 2: MID (Midfielder) - center of field
- Player 3: YOU (Left Forward)
- Player 4: FWD2 (Right Forward) - your partner on the right side
...
- Opponent GK position (agentId_0 of "away" team)
- Space around me
...
📌 SITUATION A: I HAVE THE BALL (possessionAgentId == "agentId_{MY_PLAYER_ID}")
   ...
   🎯 PRIORITY 1 - SHOOT (if close to goal):
   - If my x > 35 (inside penalty box): SHOOT immediately
   - If my x > 25 AND distance to goal < 25m: SHOOT
   - Aim technique (check opponent GK "away agentId_0" position):
     - If opp GK at y > 1: SHOOT target "LEFT" (aim y=-2)
     - If opp GK at y < -1: SHOOT target "RIGHT" (aim y=+2)
     - Else: SHOOT target "CENTER"
   - Power: always 1.0 for maximum chance
     Command: {{"commandType":"SHOOT","playerId":{MY_PLAYER_ID},"parameters":{{"power":1.0,"target":"CENTER"}},"duration":0}}

   🎯 PRIORITY 2 - PASS TO FWD2 (better positioned):
   - If FWD2 (player 4) is CLOSER to goal than me: PASS to them
   - If lane is clear and FWD2 has open space
     Command: {{"commandType":"PASS","playerId":{MY_PLAYER_ID},"parameters":{{"target_player_id":4,"power":0.7}},"duration":0}}

   🎯 PRIORITY 3 - CARRY BALL FORWARD:
   - If space ahead is clear (no opponent within 5m in front)
   - Move toward goal
     Command: {{"commandType":"MOVE_TO","playerId":{MY_PLAYER_ID},"parameters":{{"x":40.0,"y":-5.0}},"duration":0}}

   🎯 PRIORITY 4 - PASS BACK to MID:
   - If blocked and no good option: pass back to player 2 (MID)
     Command: {{"commandType":"PASS","playerId":{MY_PLAYER_ID},"parameters":{{"target_player_id":2,"power":0.6}},"duration":0}}

📌 SITUATION B: TEAMMATE HAS BALL (someone else on our team)

   🏃 OFF-THE-BALL MOVEMENT:
   - MID has ball: RUN to open space (x=+30, y=-8)
   - DEF has ball: Wait at x=+25, y=-8 (home)
   - FWD2 has ball: Support at x=+35, y=-5 (close to goal for rebound)

   🎯 MAKE DIAGONAL RUNS:
   - Move behind opponent defenders when possible
   - Create passing lanes for teammate
   - Position for potential through ball

📌 SITUATION C: OPPONENT HAS BALL

   → Move to HOME position (x=+25, y=-8)
   → Wait for counter-attack opportunity
   → DO NOT chase back to defense (that's DEF/MID's job)
   → Save stamina for attacking
...
- After FWD2 shoots: I move to x=+40, y=-3 for rebound
- After MID shoots from distance: I run to x=+40 for rebound

🎯 SHOOTING ACCURACY:
- ALWAYS check opponent GK position before shooting
- Prefer shooting from x=35-45 (close range)
- Angle matters: y between -5 and 0 gives best angle for left forward

🏃 MOVEMENT PATTERNS:
- I play on the LEFT side (y negative)
- Never go to y > +2 (that's FWD2's territory)
- Make diagonal runs toward goal
- Create width when needed (y=-10) to stretch defense

🔀 ANTI-CLUSTERING:
- Never move to a position where FWD2 is within 8m
- Keep width - I'm LEFT, FWD2 is RIGHT
- If FWD2 comes to left: I move central or wait

🚦 SCORE-BASED STRATEGY:
- Winning: play safe, hold position at x=+20 (don't over-commit)
- Losing: push forward AGGRESSIVELY, take more shots
- Tied: balanced attacking play

⏱️ TIME-BASED:
- Final 20% of match (gameTime > 80%):
  - If winning: hold position, don't lose ball
  - If losing: HIGH RISK HIGH REWARD - shoot from anywhere

⚡ FAST BREAK:
- If we JUST recovered ball in our half:
  - SPRINT forward IMMEDIATELY
  - Position at x=+35 to receive through ball
  - Don't wait - CREATE the counter
...
2. Stay in zone: Y between {ZONE_Y_MIN} and {ZONE_Y_MAX} (LEFT SIDE ONLY)
3. NEVER go to Y > 0 (that's FWD2's zone)
4. NEVER go to X < 5 (don't come back to defense)
5. Your #1 job is SHOOTING - be goal-hungry
6. When teammate has ball: make INTELLIGENT runs
7. When we don't have ball: stay in attacking position
...
def calculate_shot_target(opponent_gk_pos):
    ...
    if opponent_gk_pos["y"] > 1:
        return "LEFT"  # Opp GK is on right, shoot left
    elif opponent_gk_pos["y"] < -1:
        return "RIGHT"  # Opp GK is on left, shoot right
    return "CENTER"
...
def is_lane_open_to_goal(my_pos, opponents):
    ...
    for opp in opponents:
        ...
        # Opponent between me and goal?
        if my_pos["x"] < opp_pos["x"] < OPP_GOAL_X:
            # In the shooting lane (y-wise)
...
async def invoke(payload, context):
    log.info(f"FWD1 Agent (Player {MY_PLAYER_ID}) invoked")
    try:
